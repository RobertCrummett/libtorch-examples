...
// After how many batched to create a new checkpoint periodically
const int64_t kCheckpointEvery = 200;

// How many images to sample at every checkpoint
const int64_t kNumberOfSamplesPerCheckpoint = 10;

// Set to `true` to restore models and optimizer from previously saved
// checkpoints.
...
int main(int argc, const char* argv[]) {
	...
	parser.add_argument("--epochs").help("Number of epochs to train").default_value(int64_t{30}).scan<'i', int64_t>();
	try {
	...
	// The number of epochs to train, the default values is 30
	const int64_t kNumberOfEpochs = parser.get<int64_t>("--epochs");
	std::cout << "Training with number of epochs: " << kNumberOfEpochs << std::endl;
	...
	if (torch::cuda::is_available()) {
		std::cout << "CUDA is available. Training on the GPU." << std::endl;
	...
	// Assum the MNIST dataset is available under `kDataFolder`
	...
	if (kRestoreFromCheckpoint) {
		...
		torch::load(discriminator, "discrimiator-checkpoint.pt");
	...
	for (int64_t epoch = 1; epoch <= kNumberOfEpochs; ++epoch) {
		...
		for (const torch::data::Example<>& batch : *data_loader) {
			...
			if (batch_index % kCheckpointEvery == 0) {
				...
				// Sample the generator and save the image
