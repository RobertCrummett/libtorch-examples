...
void basic_autograd_operations_example(void) {
	...
	// Let's back propogate now. Because ``out`` contains a single scalar, ``out.backward()``
	...
	auto v = torch::tensor({0.1, 1.0, 0.00001}, torch::kFloat);
	y.backward(v);
...
void compute_higher_order_gradients_example(void) {
...
class LinearFunction : public Function<LinearFunction> {
	public:
		...
		static torch::Tensor forward(AutogradContext *ctx, torch::Tensor input, torch::Tensor weight, torch::Tensor bias = torch::Tensor()) {
...
class MulConstant : public Function<MulConstant> {
	public:
		...
		static tensor_list backward(AutogradContext *ctx, tensor_list grad_outputs) {
			// We return as many input gradients are there were arguments.
...
void custom_autograd_function_example() {
	std::cout << "====== Running \"Using custom autograd function in C++\" ======" << std::endl;
	{
		...
		auto weight = torch::rand({4, 3}).requires_grad_();
